...
- **30 invokable skills** (`/grill-me`, `/specify`, `/to-issues`, `/tdd`, `/run-acceptance`, `/debug`, `/optimize`, `/postmortem`, `/sad`, `/check-consistency`, …) that drive a disciplined workflow inside Claude Code.
...
- **Three Claude Code hooks** that cache `WebFetch`, monitor context size, and route a graceful handoff before compaction.
- **A capability system** (`capabilities/typescript`, `capabilities/typescript-hono`) that activates stack-specific rules only when your project uses that stack.
...
`ralph-local.sh` (generated by `/setup` per stack) processes the `ralph-ready` queue:
...
For each issue:
1. **Validate gates** — abort if no `scenarios:scn-*` label, if budget exhausted, or if `introduces-capability:*` is set and unreviewed.
...
3. **`/run-acceptance`** — multi-layer gate:
   ...
   - Stub detection (§106).
   - SLO benchmark (§83).
4. **`/code-review`** — invokes the `reviewer` sub-agent (§114) in a fresh context. The reviewer cites `§N` violations explicitly and posts findings.
...
6. **`gh pr create --draft`** — never `--ready`; humans approve readiness.

Token spend, time, iterations, and any rule violations land in `.planning/ralph-sessions/<YYYYMMDD>.log` (§69).
...
| §117 – §121 | Package management & supply-chain hygiene (TypeScript) |
...
|---|---|---|
| `core` (stack-agnostic) | ✅ Shipped | §1–§3, §11–§90, §122 minus stack-specific |
| `capabilities/typescript` | ✅ Shipped | §5–§10, §33, §50–§55, §117–§121 |
| `capabilities/typescript-hono` | ✅ Shipped | §38–§44 |
| `capabilities/typescript-fastify` | 🚧 Planned | Same shape, different placement rules |
| `capabilities/python` | 🚧 Planned | Twins: `§5-py`, `§52-py`, etc. |
| `capabilities/python-fastapi` | 🚧 Planned | FastAPI / SQLAlchemy / Pydantic |
| `capabilities/go` | 📋 Backlog | Go baseline |
| `capabilities/go-echo` | 📋 Backlog | Go + Echo |
