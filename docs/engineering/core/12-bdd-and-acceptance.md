...
- **A normal vertical slice in ONE bounded context is single-module (FOLLOW-UP 70).** §3 defines a module AS a bounded context, not a hexagonal layer — so a slice touching `domain/`, `application/`, and `infrastructure/` of the SAME context is one module. `detect-ceremony` collapses `src/<layer>/<ctx>` entries to `<ctx>` for the §107 count, so the common slice shape no longer trips multi-module and needs **no** override. (This is distinct from the schema-only case above, which spans ≥2 *distinct* contexts' tables and still classifies multi-module by `context_count` — the FU-66 override remains the path for that.)
