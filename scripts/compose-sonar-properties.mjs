...
// to stdout.
//
// WHICH FILE (FOLLOW-UP 13): SonarCloud reads a DIFFERENT file per analysis mode —
//   - `.sonarcloud.properties`  ← Automatic Analysis (the GitHub-App default, no CI
...
//
// Usage:
//   node scripts/compose-sonar-properties.mjs --write <capability> [capability ...]
//     # Writes BOTH .sonarcloud.properties (Automatic Analysis) and
//     # sonar-project.properties (CI scanner) in the cwd. Recommended for consumers.
//   node scripts/compose-sonar-properties.mjs <capability> [capability ...]
//     # Emits to stdout (redirect yourself: prefer `> .sonarcloud.properties` for
//     # Automatic Analysis, `> sonar-project.properties` for the CI scanner).
...
function emit(merged, capNames) {
  const lines = [
    '# Composed Sonar config — write to .sonarcloud.properties (Automatic Analysis,',
    '# the GitHub-App default) AND/OR sonar-project.properties (CI scanner). `--write`',
    '# emits both. Composed from active capabilities:',
...
if (args.length === 0) {
  console.error('Usage:');
  console.error('  node scripts/compose-sonar-properties.mjs --write <capability> [capability ...]   # writes both sonar files');
  console.error('  node scripts/compose-sonar-properties.mjs <capability> [capability ...]            # stdout');
  console.error('  node scripts/compose-sonar-properties.mjs --check <expected.properties> <capability> ...');
  process.exit(2);
}

// Both files SonarCloud may read — Automatic Analysis (.sonarcloud.properties) and
// the CI scanner (sonar-project.properties). `--write` emits both (FOLLOW-UP 13).
const SONAR_FILES = ['.sonarcloud.properties', 'sonar-project.properties'];
...
if (writeMode) {
  for (const f of SONAR_FILES) writeFileSync(f, composed);
  console.log(`✅ Wrote ${SONAR_FILES.join(' + ')} (Automatic Analysis + CI scanner) from: ${caps.map((c) => c.name).join(', ')}`);
  process.exit(0);
}
